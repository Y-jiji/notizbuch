...
The motivation of fourier analysis mainly comes from physics. In this chapter, some important motivating egs from physics are listed to provide some basic intuition. 
...
+ There are tracks on a plane #text(blue, [(blue, dashed line in @F:Vibrating_Particles_on_Tracks)])
+ These tracks are uniformly distanced #text(red, [($h$ in @F:Vibrating_Particles_on_Tracks)])
+ There are particles moving along each track #text(green, [($#circle(width: 5pt, stroke: green)$ in @F:Vibrating_Particles_on_Tracks)])
...
#fig("Vibrating Particles on Tracks")[
    ...
    #rect(width: width, height: height, inset: 0pt, outset: 0pt, stroke: none, {
        ...
        for item in data.slice(0, -1).zip(data.slice(1)) {
            ...
            place(bottom + left, dx: 0pt, dy: 0pt, line(start: i, end: j, stroke: fg-color));
        }
        // the width marker (h)
...
$
    rho h frac(pif^2 u, pif t^2) (t, x_(n)) = h^(-1)k (u(t, x_(n)+h) + u(t, x_(n)-h) - 2 dot u(t, x_(n)))
$
...
#def("Mechanical Wave")[
    ...
    $
        bold("equation") #h(10pt)
        & frac(1, c^2) dot frac(pif^2 u, pif t^2)(x, t) = frac(pif^2 u, pif x^2)(x, t) \
        & f(x) = u(x, 0) \
        & g(x) = frac(pif u, pif t)(x, t:0)\
        & u(0, t) = u(pi, t) = 0 \
        bold("where") #h(10pt)
        & u: [0,pi] times RR -> RR\
...
#rmk("D:Mechanical_Wave")[
    Regarding degree of freedom, differential expressions look just like difference expressions. 
    For eg, second order difference have $3$ free variables, which is also applicable to second order differential. 
]

#rmk("D:Mechanical_Wave", alter: "Differential Symbol")[
    The current symbols used in mathematics is very annoying. 
    For eg, when we write
    $
        frac(pif f, pif x)(y,x), frac(pif f, pif x)(x,y), frac(pif f, pif x)(y,z)
    $
    ...
    $
        frac(pif f, pif x)(y:x,x:y), frac(pif f, pif x)(a,x:b), frac(pif f, pif x)(x:y,z)
    $
]
...
#prf("T:Solution_to_MechanicalW", extra: "A")[
    ...
    $
        ...
        u(x,t) 
        = hat(u)(x-c t, x + c t) \
            frac(pif^2 u, pif x^2)(x,t) 
            - frac(1, c^2)frac(pif^2 u, pif t^2)(x,t) 
        = 4frac(pif^2 hat(u), pif xi pif zeta)(xi: x - c t,zeta: x + c t) 
        = 0
    $
    With the conditions above, we have
    $
        frac(pif hat(u), pif xi)(xi, zeta) = frac(pif hat(u), pif xi)(xi, 0)\
        hat(u)(xi, zeta)
        = integral_(xi^*=0)^(xi) frac(pif hat(u), pif xi^*)(xi^*, zeta) dif xi^* 
            + hat(u)(0, zeta)
        = hat(u)(xi,0)
            + hat(u)(0,zeta)
            - hat(u)(0,0)
    ...
    $
        cases(
            ...
            display(
                frac(pif u, pif t)(x, t:0)
                    = -c frac(pif v, pif x)(x) + c frac(pif w, pif x)(x) 
                    = g(x)
            )
        )
    $
...
#prf("T:Solution_to_MechanicalW", extra: "B")[
...
#rmk("P-B:T:Solution_to_MechanicalW")[
    Historically, some famous mathematicians like Gauss incline strongly to @P-A:T:Solution_to_MechanicalW. Their main reason is that @P-B:T:Solution_to_MechanicalW is not rigorious, because people generally don't believe functions can be decomposed into trigonometric series. This tendency lasts until, later Fourier formally proves the uniqueness of trigonometric decomposition in his study of heat flow (@H-L3:Heat_Flow). 
]

=== Heat Flow <H-L3:Heat_Flow>

While in @H-L3:Mechanical_Wave, we demonstrated how to solve differential equations with standing waves in @P-A:T:Solution_to_MechanicalW and seperation of variables in @P-B:T:Solution_to_MechanicalW, the method of standing waves are not applicable to many other differentiale equations. In this section, we introduce another physical problem: heat flow, and we will see that heat flow can be solved with seperation of variables. 

The thermal energy on a plate changes with time. Let's first suppose the plate is divided into small grids and inside seach grid, heat is transported instantly. Therefore, each grid internally have uniform temperature, as shown in @F:Heat_Flow_on_a_Plate. 

According to Newton's law of cooling, the adjacent tiles results in the following equation:
$
    kappa h^2 frac(pif u, pif t)(x_n, y_n, t) =
...
#fig("Heat Flow on a Plate")[
    // for padding
    #align(center)[#rect(stroke: none, inset: 10pt)[
        ...
        // color green
        #let cg(x) = [#set text(green); #x]
        // 4 math.display for spacing
        #let d4 = for i in range(4) { math.display(" ") }
        // 4 math.display for spacing
        #let d6 = for i in range(24) { math.display(" ") }
        // rotate 90 deg
        ...
        #jp(100% + 5pt, -250%/3,        [#cg(r9($overbrace(d6, h)$))])
        #jp(250%/3,     -100% - 5pt,    [#cg($overbrace(d6, h)$)])
    ]]
]

Write $x_(n+m)$ as $x_(n)+ m h$ and $y_(n+m)$ as $y_(n) + m h$, mimicing @H-L3:Mechanical_Wave, we can form a formal def. 

#def("Heat Flow")[
    $"HeatF"(u)$
    $
        bold("equation") #h(10pt)        
        & frac(pif u, pif t)(x,y,t) = frac(pif^2 u, pif x^2)(x,y,t) + frac(pif^2 u, pif y^2)(x,y,t)\
        bold("where") #h(10pt)
        & u:RR^3 -> RR
    $
...
$
    Delta(u) 
        = (x,y) |-> frac(pif^2 u, pif x^2)(x,y) + frac(pif^2 u, pif y^2)(x,y)
$

#thm("Polar Expression of Laplacian")[]
Let $hat(u)(r, theta) = u(r cos(theta), r sin(theta))$ where $hat(u): [0,+infinity) times [0, 2pi) -> RR$
$
    Delta(u)(r cos(theta), r sin(theta))
        = frac(pif^2 hat(u), pif r^2)(r, theta)
        + frac(1, r)frac(pif hat(u), pif r)(r, theta)
        + frac(1, r^2)frac(pif^2 hat(u), pif theta^2)(r, theta)
$

#prf("T:Polar_Expression_of_Laplacian")[Use $x=r cos(theta)$ and $y=r sin(theta)$ as macro. 
    $
    frac(pif hat(u), pif r)(r, theta) &
        = cos(theta) frac(pif u, pif x)(x,y) 
        + sin(theta) frac(pif u, pif y)(x,y)\
    $
    $
    frac(pif^2 hat(u), pif r^2)(r, theta) &
        = cos^2(theta) frac(pif u, pif x)(x,y) 
        + sin^2(theta) frac(pif u, pif y)(x,y)\
        &+ 2 sin(theta) cos(theta) frac(pif^2 u, pif x pif y)(x,y)\
    $
    $
    frac(pif hat(u), pif theta)(r, theta) &
        = -r sin(theta) frac(pif u, pif x)(x,y)
        +r cos(theta) frac(pif u, pif y)(x,y)\
    $
    $
    frac(pif^2 hat(u), pif theta^2)(r, theta)
        &= -r cos(theta) frac(pif u, pif x)(x,y)
            -r sin(theta) frac(pif u, pif y)(x,y)\
        &+ r^2 sin^2(theta) frac(pif^2 u, pif x^2)(x,y)
            +r^2 cos^2(theta)frac(pif^2 u, pif y^2)(x,y)\
        &-2r^2 sin(theta) cos(theta) frac(pif^2 u, pif x pif y)(x,y)\
    $
...
It is natural to ask if Fourier series converges to the original function, and if Fourier coefficients give a unique description of a function. But from a technical point of view, we know that changes on a finite set of points will not be reflected in Fourier coefficients, so we may have to work on weaker theorems. They either have stronger constraints on the functions (i.e. twice differential), or they have weaker conclusions (i.e. convergence doesn't apply on some points). 

#thm("Fourier Coefficients of Twice Differentiable Fn")[
    For a function $f: [a, b] -> RR$ which is twice differentiable. For each given $x$, its Fourier coefficents $hat(f)(n)$ is an $O(|n|^(-2))$ sequence. 
]

#prf("T:Fourier_Coefficients_of_Twice_Differentiable_Fn")[
    Without loss of generality, we let $a=0$ and $b=2pi$. 
    Therefore, Fourier coefficients are given by
    $
        hat(f)(n) = (2pi)^(-1) integral_0^(2pi) f(x) exp(-i n x) dif x
    $
...
#thm("Weak Convergence of FourierS")[
...
#prf("T:Weak_Convergence_of_FourierS")[
