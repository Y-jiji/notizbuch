#let build-cover(width, height, h-margin, w-margin, long-0, long-1, short-0, short-1, short-2) = {
    context[
        #set page(fill: black)
        #set text(font: "FOT-Matisse Pro", stroke: (paint: red), fill: white)
        #let long-1-size = width.pt() * 0.75 / measure(long-1).width.pt() * 1em.to-absolute();
        #place(bottom + right, dx: 0pt, dy: 0pt)[#text(long-1-size, long-1)]
        #let long-1-height = measure(text(long-1-size, long-1)).height;
        #let long-0-size  = (height - long-1-size - h-margin - 20pt) / measure(long-0).width * 1em.to-absolute();
        #place(top + right, dx: 0pt, dy: 0pt, rotate(90deg, reflow: true)[#text(long-0-size, long-0)])
